<template>
  <v-container fluid>
    <v-row justify="center" class="my-5">
      ...
      <v-col cols="12" md="8">
        <v-card class="pa-5" style="border-radius: 15px; background-color: #98FB98;" hover outlined>
            ...
            <v-row>
              ...
              <v-col cols="12" sm="6">
                <v-card style="border-radius: 20px; background-color: #ffe0e0;" class="pa-3" hover outlined>
                  ...
                  <v-card-text class="text-center">
                    <v-progress-circular :model-value="value_temp" size="90" width="20" color="red">
              ...
              <v-col cols="12" sm="6">
                <v-card style="border-radius: 20px; background-color: #e0f7fa;" class="pa-3" hover outlined>
                  ...
                  <v-card-text class="text-center">
                    <v-progress-circular :model-value="value_humi" size="90" width="20" color="blue">
                      {{ value_humi }} %
                    </v-progress-circular>
                  </v-card-text>
                </v-card>
            ...
            <v-row class="mt-5">
              ...
              <v-col cols="12" sm="6">
                <v-card style="border-radius: 20px; background-color: #fff9c4;" class="pa-3" hover outlined>
                  ...
                  <v-card-text class="text-center">
                    <v-progress-circular :model-value="totalValueT()" size="90" width="20" color="amber">
                      {{ totalValueT() }} °C
                    </v-progress-circular>
                    <v-row class="mt-3">
              ...
              <v-col cols="12" sm="6">
                <v-card style="border-radius: 20px; background-color: #c8e6c9;" class="pa-3" hover outlined>
                  ...
                  <v-card-text class="text-center">
                    <v-progress-circular :model-value="totalValueH()" size="90" width="20" color="green">
                      {{ totalValueH() }} %
                    </v-progress-circular>
                    <v-row class="mt-3">
                      <v-col cols="6" class="text-center">
    ...
    <v-row justify="center" class="my-5">
      <!-- init_revert Card -->
      <v-col cols="12" md="8">
        <v-card style="border-radius: 20px; background-color:#FFFFE0;" class="pa-5" hover outlined>
          <v-row justify="space-around" class="mb-5"> 
            <v-col cols="12" sm="5">
              <v-btn block color="#00FF00" class="white--text" large @click="start" style="border-radius: 20px; height: 50px;">
                เปิด
              </v-btn>
            </v-col>
            <v-col cols="12" sm="5">
              <v-btn block color="#FF0033" class="white--text" large @click="stop" style="border-radius: 20px; height: 50px;">
                ปิด
              </v-btn>
            </v-col>
          </v-row>
          ...
          <v-card-text>
            <v-row class="justify-center">
              <v-col cols="3" class="text-center">
                <v-icon :color="isOn ? 'green' : 'grey'" style="font-size: 30px;">
                  mdi-lightbulb-on
                </v-icon>
                <div>ON</div>
              </v-col>
              <v-col cols="2" class="text-center">
                <v-icon :color="numericValueState() === 1 || numericValueState() === 4 || numericValueState() === 5 ? 'red' : 'grey'" style="font-size: 30px;">
                  mdi-lightbulb
                </v-icon>
              </v-col>
              <v-col cols="2" class="text-center">
                <v-icon :color="numericValueState() === 2 || numericValueState() === 4 || numericValueState() === 5 ? 'green' : 'grey'" style="font-size: 30px;">
                  mdi-lightbulb
                </v-icon>
              </v-col>
              <v-col cols="2" class="text-center">
                <v-icon :color="numericValueState() === 3 || numericValueState() === 5 ? 'blue' : 'grey'" style="font-size: 30px;">
                  mdi-lightbulb
                </v-icon>
              </v-col>
              <v-col cols="3" class="text-center">
                <v-icon :color="!isOn ? 'red' : 'grey'" style="font-size: 38px;">
                  mdi-lightbulb
                </v-icon>
                <div>OFF</div>
              </v-col>
            ...
            <v-row class="justify-center">
              <v-col cols="6" class="text-center">
                <v-icon :color="temperatureStatus() === 'cold' ||  temperatureStatus() === 'quite cold' ? 'green' : 'grey'" style="font-size: 35px;">mdi-air-conditioner</v-icon>
                <div>เครื่องปรับความร้อน</div>
              </v-col>
              <v-col cols="6" class="text-center">
                <v-icon :color="humidityStatus() === 'hot' ||  humidityStatus() === 'quite hot' ? 'green' : 'grey'" style="font-size: 35px;">mdi-water</v-icon>
...
export default {
  ...
  methods: {
      ...
      start() {
          this.client.publish("init_revert", '1');
      ...
      stop() {
          ...
          this.client.publish("init_revert", '0');
          this.value_temp = 0;
          this.value_humi = 0;
      ...
      onMqttMessage(topic, message) {
          ...
          if (topic === "init_revert") {
              ...
              this.value_init_revert = message.toString();

              // let testini = message.toString();

              //  = init_data1;
              console.log("inile is ", this.value_init_revert);
              this.systemiot()
          }
      },
      updateStateshumi() {
